...
describe("combat-system", () => {
  ...
  const setupCombat = async () => {
    ...
    await playerProfileProgram.methods
      .initializeProfile("CombatTestPlayer")
      .accounts({
  ...
  describe("T019: Basic combat resolution", () => {
    it("resolves combat deterministically based on stats", async () => {
      ...
      expect(state.enemyHp).to.be.lessThan(1);
